...
//      Input variables from matlab workspace: temp ca cl cm b s
        matlab::data::TypedArray<double> temp_input = std::move(inputs[1]);
        auto temp_ptr = getPointer(temp_input);
        std::vector<size_t> size_input;
        size_input = temp_input.getDimensions();
        MatrixXd temp = Map<MatrixXd>(temp_ptr,nzbc,nxbc);
        
        matlab::data::TypedArray<double> ca_input = std::move(inputs[2]);
        auto ca_ptr = getPointer(ca_input);
        MatrixXd ca = Map<MatrixXd>(ca_ptr,nzbc,nxbc);
        
        matlab::data::TypedArray<double> cl_input = std::move(inputs[3]);
        auto cl_ptr = getPointer(cl_input);        
        MatrixXd cl = Map<MatrixXd>(cl_ptr,nzbc,nxbc);
        
        matlab::data::TypedArray<double> cm_input = std::move(inputs[4]);
        auto cm_ptr = getPointer(cm_input);
        MatrixXd cm = Map<MatrixXd>(cm_ptr,nzbc,nxbc);
        
        matlab::data::TypedArray<double> cm1_input = std::move(inputs[5]);
        auto cm1_ptr = getPointer(cm1_input);
        MatrixXd cm1 = Map<MatrixXd>(cm1_ptr,nzbc,nxbc);
        
        matlab::data::TypedArray<double> b_input = std::move(inputs[6]);
        auto b_ptr = getPointer(b_input);        
        MatrixXd b = Map<MatrixXd>(b_ptr,nzbc,nxbc);
        
        matlab::data::TypedArray<double> b1_input = std::move(inputs[7]);
        auto b1_ptr = getPointer(b1_input);        
        MatrixXd b1 = Map<MatrixXd>(b1_ptr,nzbc,nxbc);
       
        matlab::data::TypedArray<double> s_input = std::move(inputs[8]);
        auto s_ptr = getPointer(s_input);        
        VectorXd s = Map<VectorXd>(s_ptr,nt,1);
//      Eigen Initialising input variables: uu, ww, xx, xz, zz
        MatrixXd uu(nzbc,nxbc);  
        uu << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd ww(nzbc,nxbc);  
        ww << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd xx(nzbc,nxbc);  
        xx << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd xz(nzbc,nxbc);  
        xz << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd zz(nzbc,nxbc);  
        zz << MatrixXd::Zero(nzbc,nxbc);
//      Eigen Initialising input variables: fux, fuz, bwx, bwz
        MatrixXd fux(nzbc,nxbc);  
        fux << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd fuz(nzbc,nxbc);  
        fuz << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd bwx(nzbc,nxbc);  
        bwx << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd bwz(nzbc,nxbc);  
        bwz << MatrixXd::Zero(nzbc,nxbc);
//      Eigen Initialising output variables: seismo_w, seismo_u       
        MatrixXd seismo_w(nt,ng);  
        seismo_w << MatrixXd::Zero(nt,ng);
        MatrixXd seismo_u(nt,ng);  
        seismo_u << MatrixXd::Zero(nt,ng);
        
        MatrixXd wavefield_gradient_fux(nz,nx*num_nt_record);
        wavefield_gradient_fux << MatrixXd::Zero(nz,nx*num_nt_record);
        MatrixXd wavefield_gradient_fuz(nz,nx*num_nt_record);
        wavefield_gradient_fuz << MatrixXd::Zero(nz,nx*num_nt_record);
        MatrixXd wavefield_gradient_bwx(nz,nx*num_nt_record);
        wavefield_gradient_bwx << MatrixXd::Zero(nz,nx*num_nt_record);
        MatrixXd wavefield_gradient_bwz(nz,nx*num_nt_record);
        wavefield_gradient_bwz << MatrixXd::Zero(nz,nx*num_nt_record);
       
//      Eigen zero_vector for free surface zz
        VectorXd zero_vector(nxbc);
        zero_vector << VectorXd::Zero(nxbc);
        VectorXd geophone_vector(nxbc);
        geophone_vector << VectorXd::Zero(nxbc);
        ...
        double S41 = 1.1250; double S42 = -0.0416666667;
        double S61 = 1.17187; double S62 = -6.51042E-2; double S63 = 4.68750E-3;
        double S81 = 1.19629; double S82 = -7.97526E-2; double S83 = 9.57031E-3; double S84 = -6.97545E-4;
...
//        end
       ...
       double* eig_seismo_u_ptr = seismo_u.data();
       double* eig_seismo_w_ptr2 = seismo_w.data();
//        double* eig_ww_ptr2 = ww.data();
...
//        size_output.insert(size_output.begin(),nt);
       ...
       double* wavefield_gradient_fux_ptr1 = wavefield_gradient_fux.data();
       double* wavefield_gradient_fuz_ptr2 = wavefield_gradient_fuz.data();
       double* wavefield_gradient_bwx_ptr3 = wavefield_gradient_bwx.data();
       double* wavefield_gradient_bwz_ptr4 = wavefield_gradient_bwz.data();
       matlab::data::StructArray S = factory.createStructArray({ 1,1 }, { "fux", "fuz","bwx","bwz" });
       S[0]["fux"] = factory.createArray<double>(size_output_wavefield,wavefield_gradient_fux_ptr1,wavefield_gradient_fux_ptr1+wavefield_elements);//It is column-major so that the shape of fux can be automaticly reshaped in 3D matrix.
       S[0]["fuz"] = factory.createArray<double>(size_output_wavefield,wavefield_gradient_fuz_ptr2,wavefield_gradient_fuz_ptr2+wavefield_elements);
       S[0]["bwx"] = factory.createArray<double>(size_output_wavefield,wavefield_gradient_bwx_ptr3,wavefield_gradient_bwx_ptr3+wavefield_elements);
       S[0]["bwz"] = factory.createArray<double>(size_output_wavefield,wavefield_gradient_bwz_ptr4,wavefield_gradient_bwz_ptr4+wavefield_elements);
       
       outputs[0] = factory.createArray<double>(size_output,eig_seismo_u_ptr,eig_seismo_u_ptr + number_elements);
       outputs[1] = factory.createArray<double>(size_output,eig_seismo_w_ptr2,eig_seismo_w_ptr2 + number_elements);
       outputs[2] = S;
    } 
