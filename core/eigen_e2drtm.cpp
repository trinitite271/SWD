...
public:
    ...
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {
        ...
        matlab::data::TypedArray<double> seismo_w_input = std::move(inputs[8]);
        auto seismo_w_ptr = getPointer(seismo_w_input);        
        MatrixXd seismo_w = Map<MatrixXd>(seismo_w_ptr,nt,ng);
        
        
        matlab::data::TypedArray<double> wavefield_gradient_fux_input = std::move(inputs[9]);
        auto wavefield_gradient_fux_ptr = getPointer(wavefield_gradient_fux_input);
        MatrixXd wavefield_gradient_fux = Map<MatrixXd>(wavefield_gradient_fux_ptr,nz,nx*num_nt_record);
        matlab::data::TypedArray<double> wavefield_gradient_fuz_input = std::move(inputs[10]);
        auto wavefield_gradient_fuz_ptr = getPointer(wavefield_gradient_fuz_input);
        MatrixXd wavefield_gradient_fuz = Map<MatrixXd>(wavefield_gradient_fuz_ptr,nz,nx*num_nt_record);
        matlab::data::TypedArray<double> wavefield_gradient_bwx_input = std::move(inputs[11]);
        auto wavefield_gradient_bwx_ptr = getPointer(wavefield_gradient_bwx_input);
        MatrixXd wavefield_gradient_bwx = Map<MatrixXd>(wavefield_gradient_bwx_ptr,nz,nx*num_nt_record);
        matlab::data::TypedArray<double> wavefield_gradient_bwz_input = std::move(inputs[12]);
        auto wavefield_gradient_bwz_ptr = getPointer(wavefield_gradient_bwz_input);
        MatrixXd wavefield_gradient_bwz = Map<MatrixXd>(wavefield_gradient_bwz_ptr,nz,nx*num_nt_record);
        
        
//      Eigen Initialising input variables: uu, ww, xx, xz, zz
        MatrixXd uu_b(nzbc,nxbc);  
        uu_b << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd ww_b(nzbc,nxbc);  
        ww_b << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd xx_b(nzbc,nxbc);  
        xx_b << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd xz_b(nzbc,nxbc);  
        xz_b << MatrixXd::Zero(nzbc,nxbc);
        MatrixXd zz_b(nzbc,nxbc);  
        zz_b << MatrixXd::Zero(nzbc,nxbc);
//      Eigen Initialising input variables: cl_img, cm_img
        MatrixXd cl_img(nz,nx);  
        cl_img << MatrixXd::Zero(nz,nx);
        MatrixXd cm_img(nz,nx);  
        cm_img << MatrixXd::Zero(nz,nx);
        MatrixXd illum_div(nz,nx);
        illum_div << MatrixXd::Zero(nz,nx);
...
//         MatrixXd wavefield_gradient_wwb(nzbc,nxbc*nt);
//         wavefield_gradient_wwb << MatrixXd::Zero(nzbc,nxbc*nt);  
...
//        size_output_wavefield.insert(size_output_wavefield.end(),1700);//
       ...
       double* cl_img_ptr1 = cl_img.data();
       double* cm_img_ptr2 = cm_img.data();
       double* illum_div_ptr3 = illum_div.data();
//        double* wavefield_gradient_wwb_ptr = wavefield_gradient_wwb.data();//
       outputs[0] = factory.createArray<double>(size_output,cl_img_ptr1,cl_img_ptr1 + number_elements);
       outputs[1] = factory.createArray<double>(size_output,cm_img_ptr2,cm_img_ptr2 + number_elements);
       outputs[2] = factory.createArray<double>(size_output,illum_div_ptr3,illum_div_ptr3 + number_elements);
//        outputs[3] = factory.createArray<double>(size_output_wavefield,wavefield_gradient_wwb_ptr,wavefield_gradient_wwb_ptr + 23120000);//
    }};
