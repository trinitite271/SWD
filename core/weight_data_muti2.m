...
% seismo_v=bp_filter(seismo_v,dt,fsmin-5,fsmin,fsmax,fsmax+5);
[deta_cc2,deta_c,~]=RTp(seismo_v,is,df,dt,np,vmin,vmax,fmin,fmax,a,b,cr_0,dg,offset,m,FK,M,fre,ind,npair,win);
deta_cc2(ind:ind+npair-1) = deta_cc2(ind:ind+npair-1).*line_mask_r;
...
% fsmin = fsmin*df+9.8;fsmax = fsmax*df+9.8;
% seismo_v=bp_filter(seismo_v,dt,fsmin-5,fsmin,fsmax,fsmax+5);
[deta_cc22,deta_c3,~]=RTpl(seismo_v,is,df,dt,np,vmin,vmax,fmin,fmax,a,b,cr_0l,dg,offset,m,FK,M,fre,ind,npair,win);
deta_cc22(ind:ind+npair-1) = deta_cc22(ind:ind+npair-1).*line_mask_l;
